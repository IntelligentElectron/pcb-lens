...
Query nets by name pattern. Returns grouped connected pins, per-layer routing (segment counts, plus trace widths and lengths for centerline-routed copper), a compact via rollup, and layers used. Routing is read from conductor-layer copper geometry in the export; an IPC-2581 file exported without conductor/etch (cline) features returns empty routing (pins, vias, and `layersUsed` still populate).

## Description

Query nets by name pattern in an IPC-2581 file. Returns grouped connected pins, per-layer routing, and a compact via rollup (count per drill type). Routing is read from conductor-layer copper geometry in the export (`<Polyline>`/`<Line>` centerline traces and poured `<Contour>` shapes); trace widths and lengths are reported for centerline-routed copper and absent for shape/plane-routed (poured) copper, which has no centerline. If the export was generated without conductor/etch (cline) feature output, the file contains no conductor geometry and routing is empty even though pins, vias, and `layersUsed` still populate; re-export with conductor features enabled to recover widths/lengths/segment counts. Pass `detail="full"` for raw per-via coordinates (capped). Rejects patterns that match all nets.
...
Responses are token-bounded by design. By default (`detail="summary"`) the per-via coordinate array is replaced by `viaCounts` (a count per drill type + layer), so a query never balloons the caller's context. Callers that need every via coordinate pass `detail="full"`; even then the raw `viaRows` array is capped to 300 rows (with `truncated: true` set) so a `detail="full"` response stays within a tool-response budget even on the largest nets. When the cap applies, the 300 returned rows are apportioned across drill spans in proportion to each span's share of the total (rather than taken from whichever span appears first), so the truncated sample is representative of the whole net; `viaCounts` still carries the true per-span totals. The connected-pin list has its own, higher cap, so connectivity on high-fanout nets is not lost to the much smaller coordinate budget.
...
- Routing is parsed from `<Polyline>` and `<Line>` centerline conductors and from poured copper shapes (`<Contour>`/`<Polygon>`) found in conductor-layer `<LayerFeature>`/`<Set>` geometry. Centerline conductors contribute trace widths and lengths; poured shapes contribute only layer presence and a segment count (no centerline width or length), so a net poured as filled copper still reports as routed. Note this requires the IPC-2581 export to actually contain conductor geometry: some Cadence Allegro exports omit conductor/etch (cline) features and write only drill `<LayerFeature>` sections, in which case routing is empty for every net (only pins, vias, and `layersUsed` populate, the latter from `<PhyNetPoint>` layer refs). `<PhyNetPoint>` is an unordered connection-point set with no edges or width reference, so it is not used to synthesize segment counts, lengths, or widths; the fix is to re-export with conductor features enabled
- Vias are collected from `Hole` elements with `platingStatus="VIA"`. By default they are summarized as `viaCounts` (count per unique drill type + layer). With `detail="full"`, `viaRows` carry per-via coordinates and each row's `drillIndex` references `viaCounts` by position; the raw `viaRows` array is capped to 300 rows (with `truncated: true`) to keep responses token-bounded. When the cap applies, rows are sampled proportionally across drill spans so the truncated array spans every via type rather than just the first span in the file
